...
def measure_session(
    executable: Path, project: Path, queries: tuple[str, ...], timeout: float
...
def main() -> None:
    ...
    parser.add_argument("--mathlib-source", type=Path)
    ...
    executable = (args.leanreach_bin or root / ".lake/build/bin" /
                  ("leanreach.exe" if os.name == "nt" else "leanreach")).resolve()
    mathlib = (args.mathlib_source or
               project / ".lake/packages/mathlib/Mathlib").resolve()
    if not executable.exists():
        raise SystemExit("Run 'lake build' first.")
    if not mathlib.is_dir():
        raise SystemExit(
            "Mathlib source not found in the host project; use --mathlib-source."
        )

    measure_process([executable, "--help"], args.timeout, cwd=project)
    measure_process(
        [
            executable,
            "--module",
    ...
    if not args.skip_session:
        session = measure_session(executable, project, queries, args.timeout)
