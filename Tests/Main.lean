...
private unsafe def runTests : IO Unit := do
  ...
  let some cachedWrapped := planned.find? `LeanReachFixture.cachedWrapped |
    throw <| IO.userError "cached definition is missing from planned PP"
  ...
  for _ in [0:2] do
    let cachedRun ← unsafe withCachedQueryFor #[`Tests.Fixture]
      ...
      check (result.target.signature.contains "n + 1")
        "cached query lost the serialized definition body"
    unless cachedRun.isSome do
      throw <| IO.userError "definition did not use the query cache"
